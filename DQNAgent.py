        ...
        if self.training:
            ...
            if terminal:
                for t in range(self.trajectory_t):
                    self.memory.add(self.trajectory_observations[t], self.trajectory_actions[t],
                                    self.trajectory_rewards[t], (t == (self.trajectory_t - 1)))
...
def batch_objects(input_list):
    ...
    for l in input_list:
        ...
        # Create mask...
        masks.append(
            np.pad(np.array(np.ones((len(l), 1)), dtype=np.float32), ((0, max_len - len(l)), (0, 0)), mode='constant'))
    return out, masks
