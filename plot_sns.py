...
sota= { #simple, rainbow, human, random, akr3
"alien"               :( 616.9     ,739.9	        ,7127.7	     ,227.8	   , 929.6     ),
"amidar"              :( 74.3      ,188.6	        ,1719.5	     ,5.8	   , 211.52    ),
"assault"             :( 527.2     ,431.2	        ,742	     ,222.4	   , 311.64    ),
"asterix"             :( 1128.3    ,470.8	        ,8503.3	     ,210	   , 1304      ),
"bank_heist"          :( 34.2      ,51	            ,753.1	     ,14.2	   , 111.2     ),
"battle_zone"         :( 4031.2    ,10124.6         ,37187.5	 ,2360	   , 11720     ),
"boxing"              :( 7.8       ,0.2	            ,12.1	     ,0.1	   , 19.36     ),
"breakout"            :( 16.4      ,1.9	            ,30.5	     ,1.7	   , 17.8      ),
"chopper_command"     :( 979.4     ,861.8	        ,7387.8	     ,811	   , 2364      ),
"crazy_climber"       :( 62583.6   ,16185.3         ,35829.4	 ,10780.5  , 2368      ),
"demon_attack"        :( 208.1     ,508	            ,1971	     ,152.1	   , 203.2     ),
"freeway"             :( 16.7      ,27.9	        ,29.6	     ,0	       , 0.2       ),
"frostbite"           :( 236.9     ,866.8	        ,4334.7	     ,65.2	   , 402       ),
"gopher"              :( 596.8     ,349.5	        ,2412.5	     ,257.6    , 1078.4    ),
"hero"                :( 2656.6    ,6857	        ,30826.4	 ,1027	   , 6625.6    ),
"jamesbond"           :( 100.5     ,301.6	        ,302.8	     ,29	   , 182       ),
"kangaroo"            :( 51.2      ,779.3	        ,3035	     ,52	   , 1152      ),
"krull"               :( 2204.8    ,2851.5          ,2665.5	     ,1598	   , 3415.24   ),
"kung_fu_master"      :( 14862.5   ,14346.1         ,22736.3	 ,258.5    , 13604     ),
"ms_pacman"           :( 1480      ,1204.1          ,6951.6	     ,307.3	   , 4293.2    ),
"pong"                :( 12.8      ,-19.3	        ,14.6	     ,-20.7	   , -20.44    ),
"private_eye"         :( 35        ,97.8	        ,69571.3	 ,24.9	   , 116       ),
"qbert"               :( 1288.8    ,1152.9          ,13455	     ,163.9	   , 4893      ),
"road_runner"         :( 5640.6    ,9600	        ,7845	     ,11.5	   , 6204      ),
"seaquest"            :( 683.3     ,354.1	        ,42054.7	 ,68.4	   , 245.6     ),
"up_n_down"           :( 3350.3    ,2877.4	        ,11693.2	 ,533.4	   , 8045.6    ),
}
...
#df = pd.read_csv("results/ab_rep_df.csv")
num_envs = df["ENV"].nunique()
compare_var = "PROJECTION"
cols = min(num_envs, 5)
max_frames = max(df["Step"])+1
...
if True:
    ...
    for ax in g.axes.flat:
        ...
        if env_name in sota:
            ...
            ax.plot((0, max_frames), (sota[env_name][-1], sota[env_name][-1]), c="k", linewidth=lw*1.5, ls="-",
                    label="AKR2")

    handles, labels = plt.gca().get_legend_handles_labels()
    by_label = dict(zip(labels, handles))
    #plt.legend(by_label.values(), by_label.keys())
    legend = plt.figlegend(handles=by_label.values(), labels=by_label.keys(), loc="lower center", ncol=10, bbox_to_anchor=[0.5, 0], borderaxespad=0)
...
ss[["simple", "rainbow", "human", "random", "ARK2"]] = pd.DataFrame(ss["ENV"].map(sota).tolist())
