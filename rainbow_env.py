...
class EnvStacked(Env):
    def __init__(self, seed, game):
        super().__init__(seed, game, buffer_size=2)
...
class EnvDiff:
    ...
    def reset(self):
        ...
        # Process and return "initial" state
        self.lives = self.ale.lives()
        self.state_buffer.append(np.zeros_like(self.frame_buffer[0]))
        self.state_buffer.append(self._get_state())
        return self.state_buffer[1] - self.state_buffer[1]
    ...
    def step(self, action):
        ...
        for t in range(4):
            ...
            if done:
                break
        self.state_buffer.append(self.frame_buffer.max(0))

        #plt.imshow(self.state_buffer[1]-self.state_buffer[0], cmap='gray', vmin=0, vmax=255)
        #plt.show()
        # Detect loss of life as terminal in training mode
        if self.training:
            lives = self.ale.lives()
        ...
        #for r in observation:
        #    print(r)
        return self.state_buffer[1]-self.state_buffer[0], reward, done
