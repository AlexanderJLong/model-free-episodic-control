...
class Env():
  def __init__(self, seed, game):
    self.device = torch.device('cuda') #Force onto GPU (might or might not be faster should test)
  ...
  def reset(self):
    ...
    return torch.stack(list(self.state_buffer), 0).to(torch.device("cpu")).numpy()
  ...
  def step(self, action):
    ...
    return torch.stack(list(self.state_buffer), 0).to(torch.device("cpu")).numpy(), reward, done
